...
(define telaPedeNome (open-viewport "Nome do Jogador" 700 393))
(define fundoPedeNome (open-pixmap "Nome do Jogador" 700 393))
...
((draw-pixmap fundoPedeNome) "Imagens/sombraMickey.png" (make-posn 0 0))

;desenham os textos
(((draw-pixmap-posn "Imagens/Acertos.png") telaFundo)(make-posn 840 20)) ;90 790

;cria o quadro pedeNome
((draw-solid-rectangle telaPedeNome) (make-posn 70 20) 320 300 "white")
...
((draw-solid-rectangle telaFundo) (make-posn 950 500) 150 30 "white")
((draw-rectangle telaFundo) (make-posn 950 500) 150 30 (make-rgb 0 0 0))
((draw-string telaFundo) (make-posn 990 520) "RECORDE" (make-rgb 0 0 0))
...
(copy-viewport fundoPedeNome telaPedeNome)
...
(define (escreve-recorde texto)
  (cond
    [(empty? texto) ""]
    [else (string-append (first texto) "\n" (escreve-recorde (rest texto)))]))


;(define input-clique
 ; (lambda()
  ;  (let* ((click (get-mouse-click janela))
   ;        (pos (mouse-click-posn click)))
    ;  (if(and (left-mouse-click? click) (>= (posn-x pos) 950) (<= (posn-x pos) 1100 ))
     ;    (cond ((and (>= (posn-y pos) 500) (<= (posn-y pos) 530 )) ;;botao recorde
      ;          (define recorde (read-lines "ranking.txt"))
       ;         (message-box "Recorde" (escreve-recorde recorde))
        ;        (viewport-flush-input janela)
         ;       (input-clique))
          ;     (else
           ;     (viewport-flush-input janela)
            ;    (input-clique)))
...
;(define input-clique
;  ;(lambda()
;    (let ((key (ready-key-press telaFundo)))
;      (if key
;          (let ((tecla (key-value key)))
;            (cond ((or (equal? tecla #\R) (equal? tecla #\r))
;                   (define recorde (read-lines "ranking.txt"))
;                   (open-viewport "Recorde" 300 300 (escreve-recorde recorde))
;                   (viewport-flush-input telaFundo))))
;          #f)))



(define out (open-output-file "ranking.txt" #:exists 'append))
  (define in (open-input-file "ranking.txt"))

;Cria o cronometro que deixará o jogo rodar por 2 minutos
(define (jogo n s minutos click)
 (cond
   [(equal? (vector-ref acertosErros 0) 8) (begin
                                                 ((draw-string janela) (make-posn 1040 145) (number->string (pontuacaoTotal 10000))))
                                           (display (number->string (pontuacaoTotal 10000)) out)
                                                   (display " Jogador\n" out) ];290 915
   [(>= minutos 2) (begin
                     ...
                     ((draw-string janela) (make-posn 1040 145) (number->string (pontuacaoTotal 10000)) "blue"))
                     (display (number->string (pontuacaoTotal 00000)) out)
                              (display " Jogador\n" out)];posicao da pontuaçao(290 915)
   ...
   [(<= (- (current-seconds) s) 9) (cond
                                     ...
                                     [(not (equal? n (current-seconds)))
                                      (begin ((clear-solid-rectangle janela)(make-posn 1075 61) 9 18);bloco pequeno que apaga segundos de 0 a 9 (325 831)
                                             (jogo (current-seconds) s minutos (ready-mouse-release janela)))])]
   [(>= (- (current-seconds) s) 60) (begin 
                                      ((clear-solid-rectangle janela)(make-posn 1044 61) 9 18);bloco pequeno que apaga os minutos (1) (304 831)
                                      ((clear-solid-rectangle janela)(make-posn 1065 61) 18 18);bloco que apaga os segundos quando vai dar 1 minuto(325 831)
                                      ((draw-string janela) (make-posn 1044 75) (number->string (+ minutos 1)) "blue");posicao dos minutos (1)(304 845)
